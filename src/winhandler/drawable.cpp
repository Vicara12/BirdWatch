...
bool Drawable::loadTexture (std::string path, unsigned &texture, bool has_alpha,
                            unsigned border_mode)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, border_mode);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, border_mode);
  ...
  unsigned color_type = (has_alpha ? GL_RGBA : GL_RGB);

  if (image_data) {
    glTexImage2D(GL_TEXTURE_2D, 0, color_type, width, height, 0,
                 color_type, GL_UNSIGNED_BYTE, image_data);
